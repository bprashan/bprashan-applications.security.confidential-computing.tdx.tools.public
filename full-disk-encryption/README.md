...
As mentioned before, we do not make any assumption where exactly all the individual components are deployed - the components might run on the same physical machine, some components might be co-located, or each might be on a separate machine. This recipe uses QEMU direct boot, which bypasses SHIM and GRUB in the boot chain by providing the kernel and initrd directly to QEMU. This approach simplifies the boot process and reduces the attack surface.
...
7. Generate a self-signed certificate for secure HTTPS communication:
...
2. [On Preparation Host] Create TD image TD<sub>W</sub> with workload, root file system encryption key used for FDE, and key pair used for key retrieval from Trustee KBS:
    - Clone Canonical's Intel TDX repository and patch the TD launch script:
        ```
        git clone -b 3.3 https://github.com/canonical/tdx.git canonical-tdx
    ...
    - Using the (enriched) base image TD<sub>B</sub>, create an encrypted TD image TD<sub>W</sub> that is used to retrieve a TD Quote:
        ...
        - enrolls the KBS URL, the key path corresponding to root file system encryption key (<sub>KBS_k_PATH</sub>) to be stored in HashiCorp Vault, and PK_KR into an OVMF image.

        Note that changing OVMF variables does not change TD measurements.

        Script will print the updated OVMF file path and the updated encrypted image path. This OVMF file will be used for both GET_QUOTE (step 5) and TD_FDE_BOOT (runtime) modes.

        Example output:
        ```
        =============== Direct Boot LUKS Settings =============
        export UUID=<UUID of the LUKS Device>
        export label=<label of the LUKS Device>
        initrd.img-24.04 & vmlinuz-24.04 copied and placed in <Path to initrd and vmlinuz files>
        =============== Created Files ================
        OVMF_PATH: <Path to updated OVMF file>
        IMAGE_PATH: <Path to encrypted image file>
        ```

3. [**On Workload Host**] Export UUID and label returned by last command:
    ```
    export UUID=<UUID of the LUKS Device>
    export label=<label of the LUKS Device>
    ```

4. [**On Workload Host**] Clone FDE solution, Canonical's Intel TDX repository, and patch the TD launch script:
    ```
    git clone https://github.com/intel/confidential-computing-tools.git fde
    pushd fde/full-disk-encryption
    git clone -b 3.3 https://github.com/canonical/tdx.git canonical-tdx
    cp patches/boot_direct_sh.patch canonical-tdx/
    pushd canonical-tdx
    git apply boot_direct_sh.patch
    popd
    ```

5. [**On Workload Host**] Boot TD<sub>W</sub> combined with the OVMF image `OVMF_FDE.fd`.
    
    Copy the encrypted TD image, the updated OVMF binary, `initrd.img-24.04`, and `vmlinuz-24.04` from the Preparation Host to the Workload Host.
    - Place `initrd.img-24.04` and `vmlinuz-24.04` into the `canonical-tdx/guest-tools/image/` directory.
    - Update the paths for the TD image (`TD_IMG`) and the OVMF binary (`-f`) in the following command according to their location on the Workload Host.

    ```
    UUID=$UUID label=$label TD_BOOT_MODE=GET_QUOTE TD_IMG=tools/image/tdx-guest-ubuntu-24.04-encrypted.img \
        canonical-tdx/guest-tools/direct-boot/boot_direct.sh 24.04 \
        -d false \
...
6. [On Preparation Host] Export TD Quote returned by last command:
    ```
    export QUOTE=<base64 encoded TD quote>
    ```
...
1. [On Workload Host] Boot TD<sub>W</sub> combined with the OVMF image `OVMF_FDE.fd`.
    ...
    Ensure the encrypted TD image, the updated OVMF binary, `initrd.img-24.04`, and `vmlinuz-24.04` are in their correct locations on the Workload Host. `initrd.img-24.04` and `vmlinuz-24.04` should be at the `canonical-tdx/guest-tools/image/` directory. Update the paths for the TD image (`TD_IMG`) and the OVMF binary (`-f`) in the following command if they differ.
    ```
    UUID=$UUID label=$label TD_BOOT_MODE=TD_FDE_BOOT TD_IMG=tools/image/tdx-guest-ubuntu-24.04-encrypted.img \
        canonical-tdx/guest-tools/direct-boot/boot_direct.sh 24.04 \
        -d false \
        -f tools/image/OVMF_FDE.fd
    ```
